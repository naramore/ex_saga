defmodule ExSaga.Saga do
  ...
  def execute_effects_import(saga, event, opts \\ []) do
    ...
    result = case DryRun.maybe_execute(saga.effects_importer, [effects_so_far], opts) do
      {:ok, result} -> {:ok, result}
      {:error, reason} -> {:error, reason}
      otherwise -> {:error, {:unsupported_transaction_result_form, otherwise}}
    end

    Event.update(event, name: [:completed, :saga, :effects_import],
                        context: result)
  end
  ...
  def execute_compensation(saga, event, opts \\ []) do
    ...
    opts = DryRun.from_stepable(event, opts, :ok)
    result = case DryRun.maybe_execute(saga.compensation, [reason, effects_so_far], opts) do
      :ok -> :ok
      :abort -> :abort
    ...
    Event.update(event, name: [:completed, :saga, :compensation],
                        context: result)
  end
  ...
  def handle_sublevel_step(saga, event, opts) do
    ...
    case Stepable.step(stage, event, opts) do
      ...
      otherwise ->
        ...
        event = Event.update(event, name: [:completed, :saga, :stage],
                                    context: otherwise,
  ...
  @impl ExSaga.Stepper
  def handle_step(%{state: %State{hooks_left: []}} = saga, %Event{name: [:starting, :saga, :effects_import]} = event, opts) do
    event = execute_effects_import(saga, event, opts)
    {:continue, event, %{saga | state: %{saga.state | hooks_left: Hook.merge_hooks(saga.state, opts)}}}
  end
  def handle_step(%{state: %State{hooks_left: []}} = saga, %Event{name: [:completed, :saga, :effects_import]} = event, opts) do
  ...
  def handle_step(%{state: %State{hooks_left: []}} = saga, %Event{name: [:starting, :saga, :stage]} = event, opts) do
    ...
    {:continue, event, %{saga | stages_left: [stage|stages],
                                state: %{saga.state | sublevel?: true}}}
  end
  def handle_step(%{state: %State{hooks_left: []}} = saga, %Event{name: [:completed, :saga, :stage]} = event, opts) do
    case {saga, event.context} do
      ...
      {%{completed_stages: completed, stages_left: [this|left]}, {:ok, effects_so_far}} ->
        event = Event.update(event, name: [:starting, :saga, :stage],
      ...
      {%{completed_stages: []}, {status, reason, effects_so_far}} when status in [:error, :abort] ->
        event = Event.update(event, name: [:starting, :saga, :compensation],
                                    context: {status, reason, effects_so_far})
        {:continue, event, %{saga | state: %{saga.state | hooks_left: Hook.merge_hooks(saga.state, opts),
                                                          effects_so_far: effects_so_far}}}
      {%{completed_stages: [last|completed], stages_left: left}, {status, reason, effects_so_far}} when status in [:error, :abort] ->
        event = Event.update(event, name: [:starting, :saga, :stage],
                                    context: {status, reason, effects_so_far})
        {:continue, event, %{saga | state: %{saga.state | hooks_left: Hook.merge_hooks(saga.state, opts),
                                                          effects_so_far: effects_so_far},
  ...
  def handle_step(%{state: %State{hooks_left: []}} = saga, %Event{name: [:starting, :saga, :compensation]} = event, opts) do
  ...
  def handle_step(%{state: %State{hooks_left: []}} = saga, %Event{name: [:completed, :saga, :compensation]} = event, opts) do
    %{state: %{effects_so_far: effects_so_far, reason: reason}} = saga
    case event.context do
      :ok ->
      ...
      {:retry, retry_opts} ->
        case Retry.start_retry(saga.state, event, retry_opts) do
          %Event{} = event ->
            {:continue, event, %{saga | state: %{saga.state | hooks_left: Hook.merge_hooks(saga.state, opts), reason: nil}}}
          nil ->
            {:abort, reason, effects_so_far}
        end
      {:continue, effects} ->
        ...
        {:ok, effects}
      {:error, reason} ->
        event = Event.update(event, name: [:starting, :error_handler],
