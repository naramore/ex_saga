defmodule ExSaga.Retry do
  ...
  def start_retry(acc, event, retry_opts) do
    ...
    if is_nil(retry_state) do
      Event.update(event, name: [:starting, :retry, :init],
                          context: retry_opts)
    else
      Event.update(event, name: [:starting, :retry, :handler],
  ...
  def step(%{retry_updates_left: []} = acc, %Event{name: [:completed, :retry, :init]} = event, opts) do
    case event.context do
      {:ok, retry_state, retry_opts} ->
        event = Event.update(event, name: [:starting, :retry, :handler],
                                    context: {retry_state, retry_opts})
        {:continue, event, %{acc | hooks_left: Hook.merge_hooks(acc, opts)}}
      {:error, reason} ->
        %{effects_so_far: effects_so_far} = acc
  ...
  def step(%{retry_updates_left: [{path, retry}|us]} = acc, %Event{name: [:completed, :retry, :handler]} = event, opts) do
    %{update: update} = acc
    event = Event.update(event, name: [:starting, :retry, :update],
                                context: {path, retry, update})
    {:continue, event, %{acc | hooks_left: Hook.merge_hooks(acc, opts),
                               retry_updates_left: us}}
  end
  ...
  defp execute_retry_init(acc, event, opts) do
    ...
    result = case DryRun.maybe_execute(&retry_handler.init/1, [event.context], opts) do
      {:ok, retry_state, retry_opts} -> {:ok, retry_state, retry_opts}
      otherwise -> {:error, {:unsupported_retry_init_result_form, otherwise}}
    end

    Event.update(event, name: [:completed, :retry, :init],
                        context: result)
  end
  ...
  defp execute_retry_handler(acc, event, opts) do
    ...
    retry_handler = Map.get(acc, :on_retry)
    opts = DryRun.from_stepable(event, opts, {:ok, nil})
    result = case DryRun.maybe_execute(&retry_handler.handle_retry/2, [retry_state, retry_opts], opts) do
    ...
    Event.update(event, name: [:completed, :retry, :handler],
                        context: result)
  end
  ...
  defp execute_retry_update(acc, event, opts) do
    ...
    opts = DryRun.from_stepable(event, opts, {:ok, nil})
    result = case DryRun.maybe_execute(&retry.update/3, [retry_state, path, retry_result], opts) do
      :ok -> :ok
      {:ok, retry_state} -> {:ok, retry_state}
      otherwise -> {:error, {:unsupported_retry_handler_result_form, otherwise}}
    end

    Event.update(event, name: [:completed, :retry, :update],
                        context: result)
  end
end
