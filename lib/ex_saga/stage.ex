defmodule ExSaga.Stage do
  ...
  @typedoc """
  """
  @type transaction_result :: {:ok, effect} |
                              {:error, reason :: term} |
                              {:abort, reason :: term}
  ...
  @type t :: %__MODULE__{
    ...
    compensation: (reason :: term, effect_to_compensate :: effect, effects_so_far :: effects -> compensation_result),
    state: State.t,
  }
  ...
  def execute_transaction(stage, event, opts \\ []) do
    ...
    result = case DryRun.maybe_execute(stage.transaction, [effects_so_far], opts) do
      {:ok, result} -> {:ok, result}
  ...
  def execute_compensation(stage, event, opts \\ []) do
    ...
    result = case DryRun.maybe_execute(stage.compensation, [reason, effect, effects_so_far], opts) do
      :ok -> :ok
      :abort -> :abort
  ...
  def handle_step(%{state: %State{hooks_left: []}} = stage, %Event{name: [:completed, :transaction]} = event, opts) do
    ...
    case event.context do
      ...
      {status, reason} when status in [:error, :abort] ->
        event = Event.update(event, name: [:starting, :compensation],
        ...
        {:continue, event, %{stage | state: %{stage.state | hooks_left: Hook.merge_hooks(stage.state, opts), reason: reason}}}
    end
  end
    ...
    case event.context do
      ...
      {:retry, retry_opts} ->
        case Retry.start_retry(stage.state, event, retry_opts) do
          %Event{} = event ->
            {:continue, event, %{stage | state: %{stage.state | hooks_left: Hook.merge_hooks(stage.state, opts), reason: nil}}}
          nil ->
            {:abort, reason, effects_so_far}
        end
      {:continue, effect} ->
